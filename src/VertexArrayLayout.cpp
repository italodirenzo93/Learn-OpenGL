...
VertexArrayLayout::VertexArrayLayout(std::initializer_list<LayoutBinding> initializer)
...
VertexArrayLayout& VertexArrayLayout::addBinding(const LayoutBinding& binding)
{
	auto iter = std::find(m_layoutBindings.begin(), m_layoutBindings.end(), binding);
...
VertexArrayLayout& VertexArrayLayout::operator= (const VertexArrayLayout& other)
{
	glGenVertexArrays(1, &m_vertexArrayID);
...
VertexArrayLayout& VertexArrayLayout::operator= (VertexArrayLayout&& other) noexcept
{
	m_vertexArrayID = other.m_vertexArrayID;
	m_layoutBindings = std::move(other.m_layoutBindings);
