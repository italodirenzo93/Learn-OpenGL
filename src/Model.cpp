...
uint32_t textureFromFile(const char *fileName, const string &directory)
{
    ...
    int w, h, nChannels;
    auto image = stbi_load(fullPath.c_str(), &w, &h, &nChannels, STBI_default);
    if (image)
    {
        GLint format = GL_RGB;
        if (nChannels == 1)
...
vector<Mesh::Texture> Model::loadMaterialTextures(aiMaterial *material, aiTextureType textureType, const string &typeName)
{
    vector<Mesh::Texture> textures;
